...
def county_kernel_viz(locs, old_locs, model):
	...
	mia_id = I.index('12086')
	hst_id = I.index('48201')
	sea_id = I.index("53033")
	...
	def style_function(feature):        
		...
		try:
			data=(np.log(kernel[fulton_id][counties.index(str(county))]) + 12)/40 - 0.03
		except Exception as e:
			data = 0.0	
		return {
			'fillOpacity': 0.5,
	...
	#m.save("kernel/Chicago.html")
	m.save("kernel/ATL_kernel/R=1.html")
